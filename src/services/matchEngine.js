...
function calcScore(prop, req) {
  ...
  const zonas = req.zonas || [];
  if (!req.solo_por_valor && zonas.length > 0) {
    const zl = zoneLevel(prop.barrio, zonas);
    c.zona = zl === 'exact' ? w.zona : zl === 'group' ? Math.round(w.zona * 0.5) : 0;
    max.zona = w.zona;
  } else { c.zona = 0; max.zona = 0; }

  if (!req.todas_localidades && req.localidad) {
    const pl = (prop.localidad || '').trim().toLowerCase();
    const rl = req.localidad.trim().toLowerCase();
    c.localidad = pl === rl ? w.localidad : 0;
    max.localidad = w.localidad;
  } else { c.localidad = 0; max.localidad = 0; }
...
async function matchForRequirement(requirementId) {
  const reqRes = await pool.query('SELECT * FROM requirements WHERE id = $1', [requirementId]);
...
async function matchForProperty(propertyId) {
  ...
  const reqsRes = await pool.query("SELECT * FROM requirements WHERE estado = 'ACTIVO'");
